...
var chart_krm = d3.select("#chart-krm-min")
.append('svg:svg')
.attr('width', 740)
  .attr('height', 430)
  .attr('class', 'chart')
...
var tip_krm = d3.select("#chart-krm-min")
    .append('div')
    .attr('class', 'tip')
    .attr('class', 'tip')
...
x_krm.domain(kanji_data_krm_min.map(function(d) {
    return d[0];
}));
y_krm.domain([0, d3.max(kanji_data_krm_min, function(d) {
    return d[1];
})]);
...
g_krm.append("text")
  ...
  .text("Lowest 27 g-ratio for Jōyō Kanji with n-rank ≤ 1,000");

g_krm.selectAll(".bar")
    .data(kanji_data_krm_min)
    .enter().append("rect")
    // .attr("class", "bar")
    .attr("x", function(d) {
        return x_krm(d[0]);
